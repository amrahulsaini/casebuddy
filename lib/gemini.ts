...
export const ANGLE_DESCRIPTIONS: Record<string, string[]> = {
  ...
  matte: [
    ...
    'GRID 2 (Pure White Background): GENERATE A NEW REAL HUMAN HAND holding phone in EXACT case from reference in a TWISTED/TILTED position. Phone twisted at angle showing back view. Hand = NEW photorealistic human hand (NOT from reference). Case = EXACT SAME from reference (same colors, design, materials, transparency). Phone model inside case with phone back and camera visible. Hand holding phone in twisted, rotated, dynamic angle position. Clean white background.',    
