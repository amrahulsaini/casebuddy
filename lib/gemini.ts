...
export const ANGLE_DESCRIPTIONS: Record<string, string[]> = {
  ...
  bulk_doyers: [
    ...
    "PANEL 3 — BOTTOM LEFT: Two separate upright items side by side with a clear gap: empty reference case on the left, same case fitted to the phone on the right. Both items must fit fully within the cell. Same camera-opening geometry on both.",
    "PANEL 4 — BOTTOM RIGHT: Same fitted phone, full back visible, held by a freshly generated woman's hand from behind and the side edges. Fingers may appear outside the silhouette but must not cover the back window, bumper or camera. Keep the complete phone inside the cell.",
  ],
...
FRAMING: Every phone and empty case is fully visible, including its top, bottom and all four corners. Leave at least 8% clear margin between products and each cell boundary. Fit the entire group inside that margin in two-item cells. Keep the same upright orientation, realistic proportions and consistent product scale. No cropped case, stretched body, tilted divider, inset, extra cell or item crossing into another cell. No drawn grid lines or borders; separate cells with white space.
...
FINAL CHECK: Four equal cells; complete uncropped products; reference-accurate camera openings with sharp lenses; uniformly coloured phone backs with ZERO diagonal streaks, shadow overlays or reflection bands.`;
  }
