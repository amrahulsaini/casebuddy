...
export const ANGLE_DESCRIPTIONS: Record<string, string[]> = {
  ...
  transparent: [
    'PANEL 1 (Pure white #FFFFFF studio background, no cream or beige tint): TWO phones standing perfectly STRAIGHT and upright, facing the camera head-on (no tilt, no 3/4 angle, no leaning), photographed straight-on at eye level. The FRONT phone is centered and fully visible and shows its BACK inside the exact transparent case from reference, revealing the authentic phone body finish through the clear case with cameras matching specs exactly. Directly BEHIND it stands the second phone showing its FRONT screen, but it is only about 25% visible — roughly three-quarters of it is hidden behind the front phone, with just a thin sliver of its left edge and screen peeking out from behind; that screen shows realistic front glass, correct bezel and punch-hole or notch, and a tasteful unbranded abstract wallpaper, never blank white or solid black. Both phones share the same scale, lighting, and floor. Do not change case appearance.',
