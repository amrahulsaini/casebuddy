...
export const ANGLE_DESCRIPTIONS: Record<string, string[]> = {
  ...
  matte: [
    ...
    'GRID 2 (Pure White Background): GENERATE A NEW REAL HUMAN HAND holding phone in EXACT case from reference in TWISTED S-CURVE position (HYBRID DESIGN style - phone twisted, curved form). Hand = NEW photorealistic human hand. Case = EXACT SAME from reference (same colors, design, materials, transparency). Phone inside case held in twisted, curved, S-shape position by hand. Show phone back with camera through the twist. Dynamic hybrid design angle. Clean white background.',    
