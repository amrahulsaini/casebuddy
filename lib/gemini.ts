...
export const ANGLE_DESCRIPTIONS = [
  'PANEL 1 - AMAZON HERO SHOT: TWO smartphones positioned side-by-side with SLIGHT OVERLAP on CLEAN WHITE BACKGROUND. Both phones at SLIGHT 3/4 ANGLE (approximately 15-20 degree tilt) showing depth. LEFT PHONE: Shows BACK VIEW with case - BLACK FRAME clearly wrapping all edges, TRANSPARENT CENTER showing actual phone color/branding (like "vivo" logo at bottom), camera module prominently visible with precise cutouts matching the actual phone model\'s exact camera configuration (position, count, and lens arrangement as per the specified phone model name). RIGHT PHONE: Shows FRONT DISPLAY/SCREEN at matching slight 3/4 angle with screen facing viewer, displaying phone model compatibility text (like "Vivo V60 5G / Vivo T4 Pro 5G"). Both phones in upright position at IDENTICAL SLIGHT TILT ANGLE, slightly overlapping at edges, CENTERED in frame. Professional Amazon-style e-commerce studio lighting with soft shadows beneath phones. Clean white background with subtle gradient. This is the main Amazon product hero image showing complete 360° view - case design on left, screen functionality on right. Case cutouts must perfectly match the exact camera specifications based on the provided phone model name. Material contrast visible: matte black frame vs crystal-clear transparent center vs phone screen. Clean, minimal, professional product photography. Both phones ALWAYS at this consistent slight 3/4 tilt angle.',
