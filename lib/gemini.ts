...
export const ANGLE_DESCRIPTIONS: Record<string, string[]> = {
  ...
  matte: [
    'PANEL 1 (Pure White Background - Amazon Product Photography Style): EXACT empty case from reference tilted at 45-degree angle displayed on seamless white infinity sweep backdrop. CRITICAL: PRESERVE EXACT COLORS from reference - SAME frame color, SAME transparency level, SAME materials, SAME design. DO NOT CHANGE ANY COLORS. Copy colors pixel-perfect from reference image. NO phone inside. LIGHTING: 360-degree softbox lighting setup / light tent that eliminates ALL shadows completely. BACKGROUND: Pure white infinity backdrop sweep (255,255,255 RGB). POST-PROCESSING: Shadows digitally removed like Amazon product listings. 🚫 ZERO SHADOWS - NO shadows underneath, NO shadows behind, NO contact shadows, NO ambient occlusion, NO drop shadows 🚫. Completely shadowless white background. Match reference colors EXACTLY.',
    
    'PANEL 2 (Pure White Background - Amazon Product Photography Style): EXACT empty case from reference image standing upright displayed on seamless white infinity sweep backdrop - back view only showing camera cutouts. CRITICAL: PRESERVE EXACT COLORS from reference - SAME frame color, SAME transparency level, SAME materials, SAME design. DO NOT CHANGE ANY COLORS. Copy colors pixel-perfect from reference image. Show ONLY the back view displaying camera cutout design clearly. NO phone inside. NO hand. LIGHTING: 360-degree softbox lighting setup / light tent that eliminates ALL shadows completely. BACKGROUND: Pure white infinity backdrop sweep (255,255,255 RGB). POST-PROCESSING: Shadows digitally removed like Amazon product listings. 🚫 ZERO SHADOWS - NO shadows underneath, NO shadows behind, NO contact shadows, NO ambient occlusion, NO drop shadows 🚫. Completely shadowless white background. Match reference colors EXACTLY.',
  ],
};
