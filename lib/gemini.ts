...
export const ANGLE_DESCRIPTIONS: Record<string, string[]> = {
  ...
  transparent: [
    'PANEL 1 (Pure white #FFFFFF studio background, no cream or beige tint) — this is ONE single cell containing both phones together as a single photo, do NOT split into separate cells: TWO phones standing perfectly STRAIGHT and upright, facing the camera head-on (no tilt, no 3/4 angle, no leaning), photographed straight-on at eye level. The FRONT phone is centered and fully visible and shows its BACK inside the exact transparent case from reference, revealing the authentic phone body finish through the clear case with cameras matching specs exactly. There is EXACTLY ONE other phone, placed BEHIND it and shifted LEFT, showing its FRONT screen. The two overlap HEAVILY: the front phone covers about three quarters of it, so ONLY A NARROW VERTICAL SLIVER — roughly one quarter of a phone width — is visible sticking out past the LEFT edge of the front phone. They must NOT sit side by side, must NOT each take half the frame, and must NOT both be fully visible; the back phone is mostly hidden, just a thin strip. The RIGHT side of the front phone has NOTHING behind it: just clean empty white background, no second phone and no screen sliver on the right. The visible screen shows realistic front glass, correct bezel and punch-hole or notch, and a tasteful unbranded abstract wallpaper, never blank white or solid black. Both phones share the same scale, lighting, and floor. Do not change case appearance.',
