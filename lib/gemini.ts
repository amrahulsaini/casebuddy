...
export const ANGLE_DESCRIPTIONS: Record<string, string[]> = {
  ...
  bulk_black: [
    "PANEL 1 — TOP LEFT: Two upright phones, both fitted in the reference case, photographed perfectly FLAT and FRONTAL like a straight-on packshot: both phones face the camera squarely with ZERO rotation, zero yaw, zero tilt and no perspective, so neither phone shows its side wall or thickness and all four edges of each phone are straight vertical and horizontal lines. The FOREGROUND phone sits right of centre and shows its BACK: the full opaque case back with the phone's real lenses and flash seated inside the case camera openings. The SECOND phone is directly behind it, parallel to it, shifted LEFT, showing its FRONT: the case bumper wraps a realistic screen with correct bezels and front camera cutout and a soft abstract gradient wallpaper, never blank white or solid black. The whole two-phone group is only about 1.3 times as wide as a single phone. The visible strip of the second phone's screen is NARROWER than the foreground phone's camera module, about one third of a phone width at most, so more than two thirds of the second phone is hidden behind the foreground phone and its front camera cutout is hidden too. Never spread the two phones apart or show half or more of the screen. Both phones stay perfectly flat to the camera with no rotation. Nothing is behind the right side of the foreground phone.",
    ...
    "PANEL 3 — BOTTOM LEFT: The EMPTY reference case standing perfectly STRAIGHT and upright, centred, its long axis exactly vertical with no lean. The UPPER half is seen from the case's OUTSIDE BACK surface, so the raised camera plate and its openings face the viewer. In the middle the case makes exactly ONE half-twist, so the LOWER half is turned round and shows the case's inner side and bottom lip. Only one twist: never a figure-eight, double loop or inside-out upper half. Same black material, same camera openings, no phone inside, no hand. Above the case, the single word \"Flexible\" in clean black sans-serif, spelled exactly like that, appearing once, with no other words.",
...
FINAL CHECK: Four equal cells in order; complete uncropped products; the same opaque case in every cell with reference-accurate camera openings; panel 3 upright, twisted and labelled "Flexible"; panel 4 contains one wholly new generated hand with no retained reference skin.`;
  }
