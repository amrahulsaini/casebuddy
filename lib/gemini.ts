...
export const ANGLE_DESCRIPTIONS = [
  ...
  'PANEL 3 - HYBRID DESIGN 3D RENDER: SINGLE phone case displayed in 3D FLOATING POSITION on LIGHT WHITE/LIGHT GREY BACKGROUND. Case shown at DIAGONAL/ANGLED VIEW tilted approximately 30-40 degrees. Case is SLIGHTLY BENT/CURVED to show three-dimensional shape and form. Orientation is TILTED, not flat - displayed at dynamic diagonal angle. Phone case is CENTERED in frame. Shows BACK VIEW with BLACK FRAME clearly visible wrapping all edges and TRANSPARENT CENTER showing crystal clear material. Camera cutouts matching the actual phone model\'s exact camera configuration based on provided phone model name (position, count, and lens arrangement). SOFT SHADOWS beneath case for depth and dimension. Clean, professional Amazon-style product render. Text overlay on image: "HYBRID DESIGN" in bold modern font prominently displayed on the case or near it. The case should appear to be floating/suspended in 3D space with subtle rotation showing off its form and hybrid construction - combining black protective frame with transparent center panel. Light, minimal background. Clean, professional e-commerce product photography style.',
