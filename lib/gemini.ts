...
export const ANGLE_DESCRIPTIONS = [
  'PANEL 1 - HERO SHOT (Amazon Style): TWO phones in one frame - LEFT phone showing FRONT DISPLAY/SCREEN facing forward, RIGHT phone showing BACK VIEW with case at 45-degree angle. Both phones positioned on light pedestal or standing upright. Clean WHITE/LIGHT GREY studio background with subtle gradient. Right phone (back view) shows: BLACK FRAME clearly wrapping around all edges, TRANSPARENT CENTER panel displaying actual phone color and branding (like "vivo" logo) through clear material, camera module in TOP-LEFT corner prominently visible and accurately positioned. Left phone (front view) shows: screen display, giving full product view. Professional e-commerce photography with crisp shadows beneath phones. Studio lighting creates depth and dimension. Both phones positioned close together for unified composition. Camera cutouts on back phone precisely match the phone model specs. Show realistic material contrast: matte black frame vs crystal-clear transparent center. Amazon-style hero shot ready for premium product listing.',
