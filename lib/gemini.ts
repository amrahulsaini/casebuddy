...
export const ANGLE_DESCRIPTIONS: Record<string, string[]> = {
  ...
  bulk_doyers: [
    "PANEL 1 — TOP LEFT: Two upright phones, straight-on. The foreground phone shows its back fitted inside the reference case. A second complete PHONE is behind it, shifted left, mostly hidden, with a narrow strip of its illuminated front screen visible. That strip shows a blue abstract wallpaper and the real front camera cutout; never substitute an empty case, white window or blank white screen. Keep the foreground camera and back unobstructed.",
...
CAMERA GEOMETRY: Inspect the actual reference opening first. If it is one large opening, keep that one opening. If it has individual lens/flash holes, keep their exact number, placement and dimensions. Never invent a punched plate for an open-window case or replace a punched plate with an open window. The same case geometry must appear on empty and fitted cases. Fit the actual phone's lenses and flash inside the reference openings. The opaque material BETWEEN the openings stays present, in its original reference colour, on EVERY fitted phone and empty shell, including BOTH items in panel 3. Inserting a phone or rotating the case must never remove this material or reveal a differently coloured phone camera island over it. Do not add extra lens circles into flash openings. Camera rings, lens edges, flash and case rims must be individually resolved, crisp and in focus in every cell. Use deep depth of field throughout; no bokeh, blur, haze, soft focus or focus falloff.

PHONE BACK: ${bc ? `Use the requested body colour "${bc}".` : `Use one authentic factory body colour of "${phoneModel}", including pale colours when appropriate.`} Repeat the identical colour in all fitted-phone views. Show a solid opaque phone body through the clear window, with an even matte surface. The clear window contributes zero visual overlay. No diagonal streak, triangular highlight, slanted band, split light/dark region, gradient, glossy reflection, grey wash, cast shadow or hand shadow anywhere on the back or case window. Use uniform frontal illumination. Preserve the camera hardware and case edges without adding broad shading across the back. Empty windows show exactly the surrounding white background.
...
All four cells use the same sharpness and flat shadow-free lighting. No floor/contact shadows, reflections, streaks, logos, captions, watermarks or text.
...
FINAL CHECK: Four equal cells; complete uncropped products; reference-accurate camera openings with sharp lenses; uniformly coloured phone backs with ZERO diagonal streaks, shadow overlays or reflection bands. Treat silver, pearl and metallic colour names as a single matte colour swatch, never as an instruction for reflective lighting. Use the SAME colour value at the top, centre, bottom and both sides of each visible back surface; no bright edge strip, sparkling patch, metallic sheen or darker lower half. Panel 4 contains one wholly new generated hand with no retained reference skin.`;
  }
