...
- The camera housing must look IDENTICAL in every panel, empty case and phone-inserted alike: same size, same hole count, same layout, same position. Any panel where the camera cutout differs from the reference is a hard defect and the image is unusable.`;
    ...
    prompt += `\n\n${SHARPNESS_LOCK}`;
    prompt += `\n\nPHONE BODY ONLY — HIGHEST PRIORITY, APPLIES TO EVERY ONE OF THE FOUR PANELS: Through the clear window, show only the actual solid phone body in its authentic factory colour. The entire visible phone back must be one clean, evenly lit, opaque surface from edge to edge. Do NOT place, project, paint, composite, or reflect any shadow on top of the phone body — no case shadow, bumper shadow, camera-plate shadow, hand shadow, finger shadow, cast shadow, grey veil, dark patch, translucent overlay, inner shadow, ambient occlusion, vignette, reflection, glare, highlight band, diagonal streak, or light-to-dark gradient. The clear window is optically invisible over the body and must not change its colour or brightness. Preserve only the phone's real physical camera details; any shaded duplicate silhouette or dark overlay across its back is a hard defect. This rule applies independently to panels 1, 2, 3, and 4.`;
    prompt += `\n\nSHADOW-FREE CUTOUT STYLE: Use flat, uniform, on-axis frontal illumination. Do not create contact shadows or drop shadows beneath or behind the phones, cases, hand, or other items. Keep every white background uniformly #FFFFFF and keep all visible phone-body pixels free of lighting falloff.`;
