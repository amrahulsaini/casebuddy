...
export const ANGLE_DESCRIPTIONS = [
  'PANEL 1 - AMAZON HERO SHOT: TWO phones side-by-side in one frame on CLEAN WHITE/LIGHT GREY BACKGROUND. LEFT PHONE: Shows BACK VIEW at precisely 15-20 degree angle (rotated slightly toward camera) - BLACK FRAME clearly wrapping all edges, TRANSPARENT CENTER showing actual phone color/branding (like "vivo" logo), camera module prominently visible with precise cutouts matching the actual phone model\'s exact camera configuration (position, count, and lens arrangement as per the specified phone model name). RIGHT PHONE: Shows FRONT DISPLAY/SCREEN at matching 15-20 degree angle with screen facing viewer, displaying phone interface or compatibility text. Both phones PERFECTLY ALIGNED at identical angles and heights, positioned close together symmetrically. Professional e-commerce studio lighting with subtle shadows beneath phones. Light background with soft gradient. Both phones standing upright or on minimal pedestal at same level. This is the main Amazon-style product hero image showing complete 360° view - case design on left, screen functionality on right. Case cutouts must perfectly match the exact camera specifications based on the provided phone model name. Material contrast visible: matte black frame vs crystal-clear transparent center vs phone screen.',
  ...
  'PANEL 3 - PROTECTION FEATURES INFOGRAPHIC: Split diagonal layout on DARK DIAGONAL STRIPED BACKGROUND (black with subtle diagonal line pattern). Large title at top: "Built-In Camera & Screen Protection" in white bold text. TOP-RIGHT SECTION: Clear close-up of phone BACK VIEW lying FACE-UP on surface showing camera area - TRANSPARENT CENTER with actual phone color (cream/gold) visible through crystal clear material, BLACK FRAME wrapping around edges. Camera module with RAISED BLACK BEZELS/RINGS/LIP around camera module that sits HIGHER than the camera lenses - clearly visible elevated protection ring surrounding the entire camera area matching the actual phone model\'s exact camera configuration based on provided phone model name (position, count, and lens arrangement). The raised camera lip must be visibly ELEVATED above the camera lenses showing that when phone is placed face-up, lenses do NOT touch the surface. RED CIRCULAR INDICATOR highlighting the raised camera protection ring with dotted line pointing to it. White text label "Camera Protection" with arrow/pointer. BOTTOM-LEFT SECTION: Clear close-up of phone FRONT VIEW lying FACE-DOWN on surface showing screen - display visible with BLACK FRAME EDGES RAISED ALL AROUND the screen perimeter (top, bottom, left, right sides). The front edge bezels of the case must be visibly ELEVATED HIGHER than the screen surface showing that when phone is placed face-down, the screen does NOT touch the surface. Side profile view showing the raised front lip extending above screen level. RED CIRCULAR/ARROW INDICATOR highlighting the raised screen edge protection all around with dotted line pointing to it. White text label "Screen Protection" with arrow/pointer. Both sections should show CRYSTAL CLEAR SIDE-PROFILE DETAIL of protection features - camera ring elevation above lenses and screen bezel elevation above display. Professional studio lighting emphasizing depth and dimension of raised protective elements with visible shadows/gaps showing elevation. Clean minimal annotations with dotted lines and red highlights.',
