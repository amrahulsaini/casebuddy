...
function extractPrice(title, tags) {
  // Try to find price in title like "₹299" or "Rs 299"
...
async function importProducts() {
  ...
  try {
    ...
    if (categories.length === 0) {
      console.log('📝 Creating "Designer Slim Case" category...');
      const [result] = await connection.execute(
        `INSERT INTO categories (name, slug, description, parent_id, sort_order, is_active) 
         VALUES (?, ?, ?, NULL, 1, TRUE)`,
        ['Designer Slim Case', 'designer-slim-case', 'Premium designer phone cases with unique artistic designs']
      );
